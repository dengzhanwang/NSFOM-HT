...
A = randn(m,n);
x = rand(n,1);
b = exp(A*x)./(1+exp(A*x)) + 1*rand(m,1);
...
for q = -1:qmax
    ...
    for k = 1:kmax
        ...
        if q >= -1 && q<= 3
            if q == 0 % NSGD
                xp = xc - eta * mc/norm(mc)^0.7/0.8;
            elseif q == 1 % nEXTRA
                xp = xc - eta * mc/norm(mc)^0.7/1.7;
            else % nstorm
                xp = xc - eta * mc/norm(mc)^0.7/0.35;
            end
        elseif q == 4 % SGD
            xp = xc - eta * mc/14;
        elseif q ==5
...
tmp1 = (fv(:,2)-fmin)/(fv(1,2) - fmin);
semilogy(1:4:501,tmp1(1:4:end), 'LineWidth', 2.5, 'LineStyle', '-', 'Color', 'r');
hold on
...
saveas(gcf,'./result/mem_reg1_loss.png')
% title('red wine quality',FontSize=20)
...
saveas(gcf,'./result/mem_reg1_gradient.png')
1;
...
save('mem_reg1_data.mat', '-struct', 'data_to_save');
disp('数据已保存至 mem_reg1_data.mat');
