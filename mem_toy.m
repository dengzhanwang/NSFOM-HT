...
f = @(x) sum(x(1:end-1).^2 + (x(2:end) + x(1:end-1).^2).^2);


Df = @(x) [2 * x(1) + 4 * x(1) * (x(2) + x(1)^2),arrayfun(@(i) 2 * x(i) + 4 * x(i) * (x(i+1) + x(i)^2) + 2 * (x(i) + x(i-1)^2), 2:length(x)-1),2 * (x(end) + x(end-1)^2)]'... 
            + 0.0001*((1./(1 - rand(size(x)))).^(2/3) - 1).*sign(rand(size(x)) - 0.5);
...
for q = -1:qmax
    ...
    for k = 1:kmax
        ...
        elseif q == 4
            xp = xc - eta * mc/2.3;
        elseif q ==5
            xp = xc - eta * mc*min(10/norm(mc),1)/2;
        end
        if mod(k,mdata) == 0
            fvq(k/mdata+1) = f(xp);
...
figure(3)
semilogy(mv(:,2),'LineWidth',1.5);
hold on
semilogy(mv(:,3),'LineWidth',1.5);
% semilogy(mv(:,4),'LineWidth',1.5);
% semilogy(mv(:,5),'LineWidth',1.5);
semilogy(mv(:,1),'LineWidth',1.5);
semilogy(mv(:,6),'LineWidth',1.5);
semilogy(mv(:,7),'LineWidth',1.5);
%plot(fv(:,6));
legend('NSG-PM','NSG-EXTRA','NSG-STORM','SG-PM','SG-PM-Clip',FontSize=11)
xlabel('gradient evalution number',FontSize=14)
...
save('noise_data.mat', '-struct', 'data_to_save');
