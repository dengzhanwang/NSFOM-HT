...
from models.losses import CLIP_Loss, CyCLIP_Loss, SogCLR_Loss, SogCLR_DRO_Loss, VICReg_Loss, RANKCLIP_Loss
...
class CLIP(nn.Module):
    def __init__(self,               
        ...
        elif self.ita_type == 'rankclip':
                self.criterion = RANKCLIP_Loss(personalized_tau=personalized_tau, temperature=self.temp,batch_size=bsz,iteration_num=self.len_train)
    ...
    def forward(self, image, text, idx, text_idx, epoch, max_epoch,iter):
        if self.learnable_temp:
            with torch.no_grad():
        ...
        if self.ita_type in ['clip', 'cyclip']:
            ...
            if self.personalized_tau:
                image_ids = concat_all_gather(idx)
                text_ids = concat_all_gather(text_idx)
                loss_ita = self.criterion(image_feat, text_feat, iter, image_ids, text_ids)
                avg_image_tau = self.criterion.image_tau[image_ids].mean()
                avg_text_tau = self.criterion.text_tau[text_ids].mean()

            else:

                loss_ita = self.criterion(image_feat, text_feat, iter)
                if not self.learnable_temp:
                    avg_tau = torch.tensor(self.temp)
                else:
                    avg_tau = self.temp
