...
def init_dist():
    dist.init_process_group(backend='nccl')  # 使用 'nccl' 或其他后端
    torch.cuda.set_device(dist.get_rank())  # 设置每个进程使用的 GPU
...
class RANKCLIP_Loss(nn.Module):

    def __init__(self, world_size=8, temperature=0.01, personalized_tau=False, image_tau=None, text_tau=None, batch_size=128, iteration_num=1000):
        super(RANKCLIP_Loss, self).__init__()
        self.world_size = world_size
        self.temperature = temperature
        self.personalized_tau = personalized_tau  # if true, then temperatures are learnable
        self.image_tau = image_tau
        ...
        # Initialize lambda2 as a 2D tensor: (iteration_num, batch_size ** 2)
        self.lambda2 = torch.ones(batch_size**2, requires_grad=False).cuda()
        # Initialize lambda3 as a 2D tensor: (iteration_num, batch_size ** 2)
        self.lambda3 = torch.ones(batch_size**2, requires_grad=False).cuda()

    def forward(self, image_features, text_features, batch_idx, image_idx=None, text_idx=None):
        ...
        sim = torch.einsum('i d, j d -> i j', text_features, image_features) 
        simttmp = sim.T
        sim = sim - torch.min(sim, dim=1, keepdim=True)[0]  # Subtract the maximum value for numerical stability
        simT = simttmp - torch.min(simttmp, dim=1, keepdim=True)[0]
        # Apply ranking logic
        mask = torch.ones_like(sim, dtype=torch.bool)
        mask = mask.fill_diagonal_(False)  # Avoid in-place modification

        # Calculate the mean of non-diagonal elements along dim=1
        sim_non_diag = sim.masked_select(mask).view(sim.shape[0], -1)  # Remove diagonal elements
        sim_non_diag2 = simT.masked_select(mask).view(sim.shape[0], -1)

        sim_mean_dim1 = sim_non_diag.mean(dim=1)  
        sim_mean_dim0 = sim_non_diag2.mean(dim=1)  # Mean over non-diagonal elements along dim=0

        # Direction for updating lambda2 (dim=1)
        direction_lambda2 = sim - sim_mean_dim1[:, None]
        # Direction for updating lambda3 (dim=0)
        direction_lambda3 = simT - sim_mean_dim0[:,None]

        lambda2_tmp = self.lambda2.view(self.batch_size, -1).clone().detach()
        lambda3_tmp = self.lambda3.view(self.batch_size, -1).clone().detach()

        # Update only non-diagonal elements of lambda2 and lambda3
        lambda2_tmp[mask] = lambda2_tmp[mask] + 1 * direction_lambda2[mask]
        lambda3_tmp[mask] = lambda3_tmp[mask] + 1 * direction_lambda3[mask]


        # Apply the updated lambda_sum to sim
        # simtmp = sim
        simori = lambda2_tmp * sim / self.temperature
        simoriT = lambda3_tmp * simT / self.temperature
        ...
        if self.personalized_tau:
            image_temp = self.image_tau[image_idx]
            text_temp = self.text_tau[text_idx]
            total_loss = (F.cross_entropy(simori / text_temp, labels) + F.cross_entropy(simoriT/ image_temp, labels)) / 2
        else:
            total_loss = (F.cross_entropy(simori, labels) + F.cross_entropy(simoriT, labels)) / 2
