...
from optim.nresfom import NormalizedSFOMREMomentum
...
def train(model, data_loader, optimizer, tokenizer, epoch, max_epoch, warmup_steps, device, scheduler, grad_scaler):
    ...
    metric_logger.add_meter('gradient_norm', utils.SmoothedValue(window_size=1, fmt='{value:.6f}'))
    ...
    save_dir = "logs"  # 保存文件的目录
    os.makedirs(save_dir, exist_ok=True)  # 创建目录（如果不存在）
    file_name = f"{optimizer_name}CC3M.xlsx"  # 文件名
    file_path = os.path.join(save_dir, file_name)  # 文件路径

    # 如果文件存在，则读取现有数据；否则创建一个新的 DataFrame
    if os.path.exists(file_path):
        df = pd.read_excel(file_path)
    else:
        df = pd.DataFrame(columns=['epoch', 'iteration', 'loss_ita', 'gradient_norm'])
    ...
    for i, (image, text, idx, text_idx) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if grad_scaler is None:
            loss_ita, avg_image_tau, avg_text_tau, cur_eta, grad_tau_image, grad_tau_text, b_I, b_T = model(image, text_input, idx=idx, text_idx=text_idx, epoch=epoch, max_epoch=max_epoch, iter=i)
            loss_ita.backward()
...
#             loss_ita.backward()
#             gradient_norm = optimizer.step()
#         # elif isinstance(optimizer, NormalizedSFOMREMomentum):
#         #     def closure():
#         #         nonlocal loss_ita,avg_image_tau, avg_text_tau, cur_eta, grad_tau_image, grad_tau_text, b_I, b_T
#         #         optimizer.zero_grad()  # 清空梯度
#         #         model.train()
#         #         loss_ita, avg_image_tau, avg_text_tau, cur_eta, grad_tau_image, grad_tau_text, b_I, b_T = model(image, text_input, idx=idx, text_idx=text_idx, epoch=epoch, max_epoch=max_epoch,iter =i)  # 前向传播
#         #         loss_ita.backward()  # 反向传播
#         #         return loss_ita
...
def main(args):
    ...
    model = CLIP(image_encoder=args.image_encoder, text_encoder='./models/bert', embed_dim=args.embed_dim, init_model=args.init_model, bsz=args.batch_size_train*args.world_size,
                  ...
                  vicreg_sim_coeff=args.vicreg_sim_coeff, vicreg_std_coeff=args.vicreg_std_coeff, personalized_tau=args.personalized_tau, enable_surrogate=args.enable_surrogate,len_train=len(train_dataset))
    ...
    if not args.distributed:  # 这里的 args.use_distributed 是自定义的判断条件
...
if __name__ == '__main__':
    ...
    parser.add_argument('--opt', default='nextra')
    ...
    parser.add_argument('--batch_size_train', default=512)
...
#     # loss config
#     parser.add_argument('--ita_type', default='rankclip', choices=['clip','rankclip' ,'cyclip', 'vicreg', 'sogclr', 'sogclr_dro'])
#     parser.add_argument('--vicreg_sim_coeff', default=25.0, type=float)
#     parser.add_argument('--vicreg_std_coeff', default=25.0, type=float)
#     parser.add_argument('--sogclr_gamma', default=0.8, type=float)
#     parser.add_argument('--rho_init', default=8.0, type=float)
