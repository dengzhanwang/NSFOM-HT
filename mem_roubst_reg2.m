...
for q = -1:qmax
    ...
    for k = 1:kmax
        ...
        bk = b;
        if norm(xc-xm) ~=0
            diffgrad = [diffgrad norm(Df(xc,Ak,bk) - Df(xm,Ak,bk) + 2e-3*((1./(1 - rand(size(xc)))).^(1/4) - 1).*sign(rand(size(xc))  ) )/norm(xc-xm) ];
        end
        normdiff = [normdiff, norm(Df(xc,Ak,bk) - Df(xc,A,b) + 200*((1./(1 - rand(size(xc)))).^(1/4) - 1).*sign(rand(size(xc))  ) ) ];
        ...
        %         normdiff = [normdiff, norm(Df(xc,Ak2,bk2) - Df(xc,A,b) + + 00*((1./(1 - rand(size(xc)))).^(2/3) - 1).*sign(rand(size(xc)) - 0.5) )/1000 ];
        %     end
        % end
        if q == -1
        ...
        if q >= -1 && q<= 3
            if q == 0 % NSGD
                xp = xc - eta * mc/norm(mc)^0.2/70;
            elseif q == 1 % nEXTRA
                xp = xc - eta * mc/norm(mc)^0.3/100;
            else % nstorm
                xp = xc - eta * mc/norm(mc)^0.3/30;
            end
        elseif q == 4 % SGD
            xp = xc - eta * mc/400;
        elseif q ==5
...
title('QQ Plot of normdiff');
axis square; % 设置子图为正方形
% saveas(gcf,'./result/mem_robust_reg2_loss.png')
...
saveas(gcf,'./result/mem_robust_reg2_gradient.png')
1;
% figure(2)
...
save('white_wine_quality_data.mat', '-struct', 'data_to_save');
disp('数据已保存至 white_wine_quality_robust_regression_data.mat');
