...
for q = -1:qmax
    ...
    for k = 1:kmax
        ...
        if q >= -1 && q<= 3
            if q == 0 % NSGD
                xp = xc - eta * mc/norm(mc)^0.5/5;
            elseif q == 1 % nEXTRA
                xp = xc - eta * mc/norm(mc)^0.4/10;
            else % nstorm
                xp = xc - eta * mc/norm(mc)^0.5/4;
            end
        elseif q == 4 % SGD
            xp = xc - eta * mc/40;
        elseif q ==5
...
saveas(gcf,'./result/mem_reg0_loss.png')
% title('red wine quality',FontSize=20)
...
saveas(gcf,'./result/mem_reg0_gradient.png')
1;
...
save('mem_reg0_data.mat', '-struct', 'data_to_save');
disp('数据已保存至 mem_reg0_data.mat');
