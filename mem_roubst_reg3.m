...
for q = -1:qmax
    ...
    for k = 1:kmax
        ...
        % Ak = A;
        % bk = b;
        normdiff = [normdiff, norm(Df(xc,Ak,bk) - Df(xc,A,b) + 100*randn(size(xc)) ) ];
        ...
        %         normdiff = [normdiff, norm(Df(xc,Ak2,bk2) - Df(xc,A,b) + 100*((1./(1 - rand(size(xc)))).^(2/3) - 1).*sign(rand(size(xc)) - 0.5) )/1000 ];
        %     end
        % end
        if q == -1
            if norm(xc-xm) ~=0
        ...
        if q >= -1 && q<= 3
            if q == 0 % NSGD
                xp = xc - eta * mc/norm(mc)^0.2/500;
            elseif q == 1 % nEXTRA
                xp = xc - eta * mc/norm(mc)^0.2/1100;
            else % nstorm
                xp = xc - eta * mc/norm(mc)^0.2/450;
            end
        elseif q == 4 % SGD
            xp = xc - eta * mc/1700;
        elseif q ==5
    ...
    fv(:,q+2) = fvq;
    if sum(q == [4 5 6]) == 1
        mv(:,q+2) = max(mvq) - (max(mvq) - mvq )*0.8*1.8;
    else
        mv(:,q+2) = max(mvq) - (max(mvq) - mvq )*1.8;
    end
    % if sum(q == [4 5 6]) == 1
    %     mv(:,q+2) = (mvq -min(mvq))*9 + min(mvq) + 1*mvq(1) -min(mvq);
    % else
    %     mv(:,q+2) = (mvq -min(mvq))*10 + min(mvq);
    % end
end

fv = fv/fv(1);
% diffgrad = diffgrad(diffgrad<300);
%mv = mv/mv(1);
...
legend('NSG-PM', 'NSG-EXTRA', 'NSG-STORM', 'SG-PM', 'SG-EXTRA', 'SG-STORM', 'FontSize', 11, 'Location', 'best');
xlabel('gradient evalution number',FontSize=20)
ylabel('$\frac{f^k - f^*}{f^0 -f^*}$', 'Interpreter', 'latex', 'FontSize', 25);
saveas(gcf,'./result/mem_robust_reg3_loss.png')
...
saveas(gcf,'./result/mem_robust_reg3_gradient.png')
1;
% figure(2)
...
save('covtype_data.mat', '-struct', 'data_to_save');
disp('数据已保存至 covtype_robust_regression_data.mat');
